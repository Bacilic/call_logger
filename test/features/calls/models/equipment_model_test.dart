...
List<RemoteTool> _realisticCatalog() => [
  _catalogTool(id: 1, name: 'VNC', role: ToolRole.vnc, sortOrder: 1),
  _catalogTool(
    id: 3,
  ...
  _catalogTool(id: 2, name: 'AnyDesk', role: ToolRole.anydesk, sortOrder: 3),
];

void main() {
  group(
    'EquipmentModel effectiveDefaultRemoteToolId / hasInconsistentDefaultRemoteTool',
    () {
      ...
      test(
        'αναπαραγωγή 1002: default 2 αλλά μόνο VNC επιλεγμένο → ασυνέπεια',
        () {
      ...
      test('κενό defaultRemoteTool → όχι ασυνέπεια', () {
        final eq = EquipmentModel(remoteParams: {'1': 'x'});

        expect(eq.hasInconsistentDefaultRemoteTool(catalog), isFalse);
      });

      test(
        'μη-αριθμητικό defaultRemoteTool → όχι ασυνέπεια (χειρίζεται ο resolver)',
        () {
          final eq = EquipmentModel(
      ...
      test('αποθηκευμένο id χωρίς επιλεγμένο εργαλείο → ασυνέπεια', () {
        final eq = EquipmentModel(defaultRemoteTool: '2', remoteParams: {});
      ...
      test(
        'κενή τιμή παραμέτρου στο remote_params αγνοείται στο effective',
        () {
          final eq = EquipmentModel(
            defaultRemoteTool: '1',
      ...
      test(
        'toMap περιλαμβάνει null default_remote_tool για εκκαθάριση στη βάση',
        () {
      ...
      group('displayPrimaryRemoteToolId (υπολογιζόμενο κύριο για τη λίστα)', () {
        ...
        test(
          'αγνοεί το αποθηκευμένο default_remote_tool (1002: default 2 → δείχνει 1)',
          () {
            final eq = EquipmentModel(
              defaultRemoteTool: '2',
              remoteParams: {'1': '4324324'},
