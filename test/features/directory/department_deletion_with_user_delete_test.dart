...
void main() {
  ...
  test('διαγραφή τμήματος με διαγραφή υπαλλήλου: ατομικό + πλήρες undo',
      () async {
    ...
    final phoneP =
        await db.insert('phones', {'number': 'P111', 'is_deleted': 0});
    ...
    await db.insert('user_equipment', {'user_id': userA, 'equipment_id': equipE});
    ...
    expect(await intVal('SELECT is_deleted FROM departments WHERE id=?',
            [deptDel]),
        1);
    expect(
        await intVal('SELECT department_id FROM users WHERE id=?', [userB]),
        dept2);
    expect(await intVal('SELECT is_deleted FROM users WHERE id=?', [userA]), 1);
    expect(await intVal('SELECT is_deleted FROM phones WHERE id=?', [phoneP]),
        1);
    expect(
        await intVal('SELECT department_id FROM equipment WHERE id=?', [equipE]),
        dept2);
    expect(
        await intVal('SELECT COUNT(*) FROM user_phones WHERE user_id=?', [userA]),
        0);
    expect(
        await intVal(
            'SELECT COUNT(*) FROM user_equipment WHERE user_id=?', [userA]),
        0);
    ...
    expect(
        await intVal('SELECT is_deleted FROM departments WHERE id=?', [deptDel]),
        0);
    expect(await intVal('SELECT department_id FROM users WHERE id=?', [userB]),
        deptDel);
    expect(await intVal('SELECT is_deleted FROM users WHERE id=?', [userA]), 0);
    expect(await intVal('SELECT is_deleted FROM phones WHERE id=?', [phoneP]),
        0);
    expect(
        await intVal('SELECT COUNT(*) FROM user_phones WHERE user_id=? AND phone_id=?',
            [userA, phoneP]),
        1);
    expect(
        await intVal('SELECT department_id FROM equipment WHERE id=?', [equipE]),
  ...
  test(
    ...
    final phoneP =
        await db.insert('phones', {'number': 'P222', 'is_deleted': 0});
    await db.insert('user_phones', {'user_id': userA, 'phone_id': phoneP});
    ...
    expect(
        await intVal(
            'SELECT is_deleted FROM departments WHERE id=?', [createdDeptId]),
        1);
    expect(await intVal('SELECT department_id FROM users WHERE id=?', [userB]),
        deptDel);
    expect(await intVal('SELECT is_deleted FROM users WHERE id=?', [userA]), 0);
