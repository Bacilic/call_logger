...
void main() {
  ...
  group('Αναγνωριστικά Lansweeper', () {
    ...
    test(
      'τμήμα: ένα μήνυμα ΑΝΑ προβληματικό λογαριασμό — όχι συγκεντρωτικό',
      () {
    ...
    test('ανενεργός κανόνας → σιωπή παντού', () {
      ...
      expect(s.lansweeperDomainMismatchProblems(r'Α = 3gnk\a', 'gnk'), isEmpty);
    });

    test(
      'scan: άκυρο αναγνωριστικό υπαλλήλου → εύρημα με εστίαση στο πεδίο',
      () {
        final findings = service.scan(
          users: [
            UserModel(
