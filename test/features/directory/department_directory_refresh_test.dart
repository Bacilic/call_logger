...
void main() {
  ...
  group('DepartmentDirectoryNotifier — sibling catalog refresh', () {
    ...
    test(
      'bulkUpdate — ενημέρωση ονόματος τμήματος σε κοινόχρηστο τηλέφωνο',
      () async {
    ...
    test(
      'deleteSelected — αφαίρεση ονόματος τμήματος από κοινόχρηστο τηλέφωνο',
      () async {
        final container = await _container();
        final deptId = await _seedDepartmentId();
    ...
    test(
      'undoLastDelete — επαναφορά ονόματος τμήματος σε κοινόχρηστο τηλέφωνο',
      () async {
        final container = await _container();
        final deptId = await _seedDepartmentId();
    ...
    test(
      'restoreDepartmentByName — επαναφορά soft-deleted τμήματος στον κατάλογο τηλεφώνων',
      () async {
        final container = await _container();
        final deptId = await _seedDepartmentId();
        ...
        await container.read(lookupServiceProvider.future);
        await container.read(directoryProvider.notifier).loadUsers();
        final labelAfterSoftDelete = _orphanPhoneDeptLabel(
          container,
          orphanPhone,
        );
        expect(
