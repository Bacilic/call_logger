...
Future<void> _pumpUntilEquipmentSaveCompletes(WidgetTester tester) {
  return pumpUntilDialogCloses(
    ...
    failMessage: 'Η φόρμα εξοπλισμού δεν έκλεισε εγκαίρως μετά την αποθήκευση',
  );
}
  ...
  group('Φόρμα εξοπλισμού — χαρακτηρισμός (widget)', () {
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(
          find.text('Κενό = στέλνεται αυτόματα η τιμή PC1002'),
          findsOneWidget,
          reason: greekExpectMsg(
            'Το κείμενο βοήθειας ονομάζει την τιμή που θα σταλεί, ώστε να '
            'φαίνεται χωρίς να χρειάζεται εστίαση στο πεδίο',
          ),
        );
      },
    );
