...
void main() {
  group('lampResolutionEtaText', () {
    test('90 δευτερόλεπτα → 1:30', () {
      expect(lampResolutionEtaText(const Duration(seconds: 90)), '1:30');
    });

    test('45 δευτερόλεπτα → 0:45', () {
      expect(lampResolutionEtaText(const Duration(seconds: 45)), '0:45');
    });
  });
  ...
  group('LampResolutionProgressDialog', () {
    ...
    testWidgets('η μπάρα προόδου είναι καθορισμένη (processed / totalSteps)', (
      tester,
    ) async {
      final logController = ResolutionLogController();
    ...
    testWidgets('χρήστης στο τέλος — νέα εγγραφή log κάνει άλμα στο τέλος', (
      tester,
    ) async {
      final logController = ResolutionLogController();
      final progress = ValueNotifier<int>(0);
      ...
      for (var i = 0; i < 40; i++) {
        logController.add(ResolutionLogEntry.info('Γραμμή log $i ${'x' * 80}'));
      }
      await tester.pump();
      await tester.pump();
    ...
    testWidgets('χρήστης ψηλότερα — νέα εγγραφή log δεν μετακινεί την κύλιση', (
      tester,
    ) async {
      final logController = ResolutionLogController();
      final progress = ValueNotifier<int>(0);
