...
void main() {
  ...
  group('AuditService keyword search', () {
    ...
    test(
      '«θεσης» δεν επιστρέφει εγγραφή με μόνο «αλλαγη προθεσμιας»',
      () async {
    ...
    test(
      '«παραμε» δεν επιστρέφει «αντιγραφο ασφαλειας παραλειφθηκε»',
      () async {
        await insertAuditRow(
          db,
          searchText: SearchTextNormalizer.normalizeForSearch(
    ...
    test(
      '«θεση» βρίσκει «αλλαγη θεσης x» και «θεσης» βρίσκει «θεση»',
      () async {
        ...
        expect(positionPage.rows.map((r) => r['id']).toSet(), {
          positionRow,
          plainRow,
        });
        ...
        expect(casePage.rows.map((r) => r['id']).toSet(), {
          positionRow,
          plainRow,
        });
      },
  ...
  group('AuditService search_text index', () {
    ...
    test(
      'rebuildAllSearchTexts ανακατασκευά idempotent το search_text',
      () async {
        ...
        try {
          ...
          final id = await db.insert('audit_log', {
            ...
            'old_values_json': jsonEncode({'color': '#1976D2', 'map_x': 0.5}),
            'new_values_json': jsonEncode({'color': '#EF5350', 'map_x': '0.5'}),
          });
