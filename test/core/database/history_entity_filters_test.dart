// Το Ιστορικό φιλτράρει σε τμήμα, καλούντα και εξοπλισμό — όπως τα Στατιστικά.
//
...
void main() {
  ...
  group('Φίλτρα οντότητας στο Ιστορικό', () {
    ...
    test('ο καλών βρίσκεται από το όνομα που δείχνει η κλήση', () async {
      await seedCalls();
      expect(await issuesWhere(userName: 'Βαρβάρα Ψαρρά'), ['Δεν ανοίγει']);
    });

    test('ο καλών είναι φίλτρο προσώπου, όχι αναζήτηση κειμένου', () async {
      await seedCalls();

      // Μέρος ονόματος: η τιμή βγαίνει από κλειστή λίστα, οπότε δεν υπάρχει
      // «μισό όνομα» — το πεδίο προτείνει ολόκληρο τον άνθρωπο.
      expect(await issuesWhere(userName: 'Ψαρρά'), isEmpty);

      // Το τηλέφωνο ψάχνεται μέσα στη λίστα προτάσεων και φεύγει ως όνομα.
      expect(await issuesWhere(userName: '2540'), isEmpty);

      // Και το σοβαρό: λέξη που δεν είναι άνθρωπος δεν φιλτράρει ποτέ.
      expect(
        await issuesWhere(userName: 'Αιματολογικό'),
        isEmpty,
        reason:
            'Το τμήμα ζει στο ίδιο κείμενο αναζήτησης με τον καλούντα· αν το '
            'φίλτρο προσώπου το δεχόταν, θα έφερνε ξένες κλήσεις.',
      );
    });
