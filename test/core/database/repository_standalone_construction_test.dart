...
void main() {
  group('Repository standalone construction — single-arg db', () {
    ...
    test(
      'CategoryRepository(db).getCategoryNames() / getActiveCategoryRows()',
      () async {
    ...
    test(
      'EquipmentRepository(db).getEquipmentDefaultRemoteToolUsageCounts()',
      () async {
        ...
        final counts = await EquipmentRepository(
          db,
    ...
    test(
      'DepartmentRepository(db).backfillDepartmentFloorIdsFromMapFloor()',
      () async {
        ...
        final count = await DepartmentRepository(
          db,
    ...
    test('DepartmentRepository(db).backfillAllDepartmentNameKeys()', () async {
      ...
      final result = await DepartmentRepository(
        db,
    ...
    test(
      'CategoryRepository(db).softDeleteCategories / restoreCategories',
      () async {
        final repo = CategoryRepository(db);
        ...
        expect(
          restoreAudits.single['user_performing'],
    ...
    test('DepartmentRepository(db) — CRUD/audit standalone paths', () async {
      final repo = DepartmentRepository(db);

      expect(await repo.departmentNameExistsExcluding('Άγνωστο', 999), isFalse);
      ...
      expect(
        await repo.departmentNameExistsExcluding(
          'Standalone Dept CRUD',
          id + 999,
        ),
        isTrue,
    ...
    test(
      ...
      () async {
        ...
        expect(
          (await equipment.getAllEquipment()).any((r) => r['id'] == eqId),
          isTrue,
        );
        ...
        expect(
          links.any((r) => r['user_id'] == userId && r['equipment_id'] == eqId),
          isTrue,
        );
        ...
        await db.delete('audit_log');
        await equipment.bulkUpdateEquipments(
          [eqId],
          {'notes': 'Bulk standalone'},
        );
    ...
    test('Tier 4d-2 — User/Department/Phone/Equipment query paths', () async {
      final users = UserRepository(db);
      final departments = DepartmentRepository(db);
      final phones = PhoneRepository(db);
      final equipment = EquipmentRepository(db);
      ...
      final deptId = await db.insert('departments', {
        ...
        'name_key': SearchTextNormalizer.normalizeForSearch('Τμήμα Tier 4d-2'),
        'is_deleted': 0,
      });
    ...
    test('Tier 4d-3 — Department/User/BuildingMap widget paths', () async {
      final departments = DepartmentRepository(db);
      final users = UserRepository(db);
      ...
      await db.insert('departments', {
        ...
        'name_key': SearchTextNormalizer.normalizeForSearch('Τμήμα Tier 4d-3'),
        'is_deleted': 0,
      });
      ...
      final createdId = await departments.getOrCreateDepartmentIdByName(
        'Νέο Tier 4d-3',
      );
      expect(createdId, isNotNull);
      final reusedId = await departments.getOrCreateDepartmentIdByName(
        'νέο tier 4d-3',
      );
    ...
    test('Tier 5b — IntegrityService(db) integrity + task paths', () async {
      ...
      expect(
        await integrity.integrityUserLabel(db, userId),
        contains('Integrity Standalone'),
      );
      ...
      final oldRow = await integrity.integrityUpdateTaskFk(
        db,
        taskId,
        'caller_id',
        null,
    ...
    test(
      ...
      () async {
        ...
        expect(
          (await users.getAllUsers()).any((r) => r['id'] == newUserId),
          isTrue,
        );
        expect(
          (await equipment.getAllEquipment()).any((r) => r['id'] == eqId),
    ...
    test('Tier 6 — Lamp migration repository paths', () async {
      final users = UserRepository(db);
      final equipment = EquipmentRepository(db);
      final departments = DepartmentRepository(db);
