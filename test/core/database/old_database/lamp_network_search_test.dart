...
void main() {
  ...
  group('καθολική αναζήτηση στα πεδία δικτύου', () {
    test(
      'βρίσκει εξοπλισμό με IP, MAC, hostname, VLAN και σχόλια δικτύου',
      () async {
  ...
  group('updateSection για την κάρτα Δίκτυο', () {
    ...
    test('γραμμή με παλιό ελάττωμα set_master→εαυτό: οι σημειώσεις δικτύου '
        'αποθηκεύονται κανονικά', () async {
    ...
    test(
      'η αλλαγή του ίδιου του set_master σε εαυτό απορρίπτεται ακόμη',
      () async {
        await seedWithNetworkFields();
        final result = await repository.updateSection(
          databasePath: dbPath,
