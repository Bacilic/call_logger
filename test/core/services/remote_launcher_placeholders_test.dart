...
void main() {
  ...
  group('RemoteLauncherService.testArgumentList', () {
    ...
    test('EQUIPMENT_CODE και TARGET από δοκιμαστικό host', () {
      ...
      expect(RemoteLauncherService.testArgumentList(tool, '922'), [
        '-host=PC922',
        '-password=pass99',
      ]);
      expect(RemoteLauncherService.testArgumentList(tool, 'pc922'), [
        '-host=PCpc922',
        '-password=pass99',
      ]);
    });
    ...
    test('παραλείπει ανενεργά ορίσματα', () {
      final tool = vncTool(const [
        RemoteToolArgument(value: '-host=PC{EQUIPMENT_CODE}', isActive: true),
        RemoteToolArgument(value: '-password=secret', isActive: false),
      ]);
      expect(RemoteLauncherService.testArgumentList(tool, '922'), [
        '-host=PC922',
      ]);
  ...
  group('RemoteLauncherService.formatTestCommandPreview', () {
    test('ενιαία γραμμή εντολής με ενεργά ορίσματα', () {
      final tool = RemoteTool(
        ...
        arguments: const [
          RemoteToolArgument(value: '-host=PC{EQUIPMENT_CODE}', isActive: true),
          RemoteToolArgument(value: '-password=12345', isActive: true),
        ],
      );
      expect(
  ...
  group(
    'RemoteLauncherService — ένας στόχος ανά εκτέλεση ({FILE} vs {TARGET})',
    () {
      ...
      test(
        ...
        () {
          ...
          final args = RemoteLauncherService.testArgumentList(
            tool,
            '10.0.0.55',
          );
          expect(
      ...
      test('με {TARGET}+{FILE} η λίστα δοκιμής περιέχει μόνο το αρχείο', () {
        ...
        expect(RemoteLauncherService.testArgumentList(tool, '10.0.0.55'), [
          kPreviewRdpFilePath,
        ]);
      });

      test('όρισμα με {TARGET} και {FILE} στην ίδια τιμή διατηρείται', () {
        final tool = rdpTool(const [
          RemoteToolArgument(value: r'/f:{FILE} /v:{TARGET}', isActive: true),
          RemoteToolArgument(value: '{TARGET}', isActive: true),
        ]);
        expect(RemoteLauncherService.testArgumentList(tool, '10.0.0.55'), [
  ...
  group('RemoteTool.acceptsFileParam', () {
    test(
      'αναγνωρίζει ενεργό placeholder αρχείου ανεξάρτητα από πεζά/κεφαλαία',
      () {
        final t = RemoteTool(
          id: 10,
          name: 'RDP Template',
    ...
    test('δεν αναγνωρίζει ανενεργό placeholder αρχείου', () {
      final t = RemoteTool(
        ...
        sortOrder: 1,
        isActive: true,
        arguments: const [RemoteToolArgument(value: '{FILE}', isActive: false)],
      );
