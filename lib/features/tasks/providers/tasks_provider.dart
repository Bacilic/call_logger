...
/// Ξαναδιάβασμα των εκκρεμοτήτων όταν αλλάζει ο κατάλογος (στρώμα provider).
///
/// Η λίστα δεν κουβαλά μόνο τις ίδιες τις εκκρεμότητες: κουβαλά και **σημαίες
/// διαγραφής** για τον καλούντα, το τμήμα και τον εξοπλισμό της καθεμιάς. Οι
/// σημαίες παγώνουν τη στιγμή της φόρτωσης, οπότε μια διαγραφή που γίνεται
/// αργότερα δεν φτάνει ποτέ στην κάρτα: ο εξοπλισμός εμφανίζεται ζωντανός και
/// το κουμπί επεξεργασίας ανοίγει το κενό.
///
/// Γιατί εδώ και όχι δίπλα στο ξέπλυμα των Κλήσεων: εκείνο ζει υποχρεωτικά στο
/// widget layer (κύκλος εξαρτήσεων). Οι εκκρεμότητες δεν ανήκουν σε εκείνη την
/// αλυσίδα, οπότε επιβάλλονται **μία φορά** στο σημείο απ' όπου περνούν όλες οι
/// μεταλλάξεις καταλόγου — και όχι μία φορά ανά καρτέλα.
///
/// Η ακύρωση γίνεται **αμέσως**, χωρίς αναβολή σε επόμενο καρέ: ο καλών βρίσκεται
/// πάντα μετά από `await` (εκτός φάσης χτισίματος) και ακυρώνει ήδη διπλανό
/// provider με τον ίδιο τρόπο. Η αναβολή θα απαιτούσε μηχανή καρέ, την οποία τα
/// καθαρά unit tests δεν έχουν — και θα έσκαγαν χωρίς λόγο.
void invalidateTasksAfterDirectoryMutation(Ref ref) {
