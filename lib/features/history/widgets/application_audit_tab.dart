...
class _ApplicationAuditTabState extends ConsumerState<ApplicationAuditTab> {
  ...
  /// Όταν αλλάζει το περιεχόμενο της λίστας ως σύνολο (σελίδα ή φίλτρα),
  /// η προβολή ξαναρχίζει από την κορυφή.
  ...
  Future<void> _confirmAndDeleteSelection() async {
    ...
    // Μέσω container (όχι widget ref): η εκτέλεση δεν εξαρτάται από τη ζωή
    // της οθόνης — ό,τι υποσχέθηκε το snackbar θα γίνει, κι ας αλλάξει οθόνη.
    ...
    scheduleDeferredDeletionWithUndo(
      ...
      countdownMessage: (s) => deleteIds.length == 1
          ? 'Η εγγραφή θα διαγραφεί οριστικά σε $s″.'
          : 'Οι ${deleteIds.length} εγγραφές θα διαγραφούν οριστικά σε $s″.',
      completedMessage: deleteIds.length == 1
          ? 'Η εγγραφή διαγράφηκε οριστικά.'
          : 'Διαγράφηκαν οριστικά ${deleteIds.length} εγγραφές.',
      failureMessage: 'Η διαγραφή απέτυχε — οι εγγραφές παραμένουν.',
      execute: () async {
        final service = await container.read(auditServiceAsyncProvider.future);
        await service.deleteByIds(deleteIds);
      },
      onFinished: (_) {
        container.invalidate(auditListProvider);
        if (mounted) setState(() => _deleteCountdownActive = false);
      },
      onUndo: () {
        if (mounted) setState(() => _deleteCountdownActive = false);
      },
                    ...
                    Expanded(
                      child: LayoutBuilder(
                        builder: (context, constraints) {
                          ...
                          final actionField = AuditFilterAutocomplete(
                            key: const ValueKey('audit-filter-action'),
                            labelText: 'Ενέργεια',
                            options: actionOptions
                                .map(
                                  (action) => AuditFilterAutocompleteOption(
                                    value: action,
                                    label: action,
                                  ),
                                )
                          ...
                          final entityDropdown = AuditFilterAutocomplete(
                  ...
                  data: (AuditPageResult r) {
                    ...
                    return ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ListTile(
                          selected: selected,
                          // Ορατό υπόστρωμα επιλογής: τίτλος/υπότιτλος/εικονίδιο
                          // έχουν δικά τους χρώματα, οπότε το selected μόνο του
                          // δεν αποδίδει καμία ορατή διαφορά.
