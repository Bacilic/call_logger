...
class RemoteToolFormSaver {
  ...
  Future<RemoteTool?> findSoftDeletedConflict(String name, {int? excludeId}) =>
  ...
  Future<int> commitNew({required RemoteTool toolFromForm}) async {
    ...
    await _repo.reorderToolToPosition(toolId: newId, positionOneBased: n + 1);
    return newId;
  }

  Future<void> commitEdit({required RemoteTool toolFromForm}) async {
    final fresh = await _repo.getAllNonDeletedTools();
