...
class CallHeaderState {
  ...
  bool get hasPhoneAssociation {
    ...
    // Ελέγχουμε αν το επιλεγμένο τηλέφωνο περιέχεται στα τηλέφωνα του χρήστη (μπορεί να έχει πολλά, π.χ. διαχωρισμένα με κόμμα)
    return callerPhone.split(RegExp(r'[,\s/]+')).contains(selPhone) || callerPhone.contains(selPhone);
  }
...
class CallHeaderNotifier extends Notifier<CallHeaderState> {
  ...
  /// Μηδενίζει selectedPhone, selectedCaller, selectedEquipment, phoneError και candidates (για clear button).
  void clearAll() {
    state = state.copyWithClearSelections();
  }
  ...
  Future<void> performPhoneLookup(String phone) async {
    ...
    if (users.length == 1) {
      ...
      if (users.first.id != null) {
        performEquipmentLookup(users.first.id!);
      }
      return;
    }
    state = state.copyWith(
