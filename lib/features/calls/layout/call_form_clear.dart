...
///
/// CONTRACT (μην αλλοιωθεί): η εκκαθάριση είναι ΜΙΑ αδιαίρετη ενέργεια πέντε
/// βημάτων. Αν παραλειφθεί έστω ένα, μένουν «κολλημένες» επιβεβαιώσεις πεδίων,
/// μάνταλο μεγάλης προβολής από ακυρωμένη κλήση, ή — χειρότερα — σημειώσεις της
/// προηγούμενης κλήσης που ξαναεμφανίζονται στην επόμενη. ΚΑΘΕ σημείο που
/// καθαρίζει τη φόρμα (κουμπί «Εκκαθάριση», κόκκινο ×, διάλογος-φρουρός αλλαγής
/// βάσης, μελλοντικές ροές) οφείλει να καλεί **μία από τις δύο** συναρτήσεις
/// αυτού του αρχείου αντί να επαναλαμβάνει τα βήματα.
...
/// Ίδια εκκαθάριση, αλλά η οθόνη **μένει ανοιχτή** — το κόκκινο × των πεδίων.
///
/// Η μόνη διαφορά από την πλήρη είναι το μάνταλο: το × το πατά κάποιος που
/// θέλει να ξαναγράψει αμέσως, και η σύμπτυξη θα του έκλεινε τη φόρμα μπροστά
/// στα μάτια. Γράφεται εδώ, δίπλα στην πλήρη, ώστε οι δύο παραλλαγές να μην
/// μπορούν να αποκλίνουν: ό,τι προστεθεί στην εκκαθάριση το κερδίζουν και οι δύο.
void clearCallFormKeepingScreenExpanded(WidgetRef ref) {
