...
class SettingsRepository {
  ...
  ///
  /// Μία χαμένη κούρσα είναι φυσιολογική· πέντε στη σειρά σημαίνει μηχάνημα
  /// που γράφει ασταμάτητα, κατάσταση που δεν συμβαίνει στην πράξη.
  static const int _updateAttempts = 5;

  /// **Στοχευμένη αλλαγή** — ο σωστός τρόπος για ρύθμιση που στοιβάζει πολλές
  /// επιλογές σε ΜΙΑ τιμή (JSON ή λίστα).
  ///
  /// Οι απλές ρυθμίσεις είναι κλειδί-τιμή, οπότε δύο διαχειριστές που αλλάζουν
  /// **διαφορετικά** πράγματα δεν ενοχλούν ο ένας τον άλλον. Όπου όμως πολλές
  /// επιλογές ζουν σε ένα κλειδί, η [saveSetting] ξαναγράφει την τιμή ολόκληρη
  /// από την εικόνα που κρατά η οθόνη — και η αλλαγή του πρώτου εξαφανίζεται.
  ///
  /// Η [change] δέχεται την **τρέχουσα αποθηκευμένη** τιμή (`null` = δεν
  /// υπάρχει ακόμη) και επιστρέφει τη νέα. Η εγγραφή γίνεται με ατομική
  /// δέσμευση: αν κάποιος πρόλαβε, η [change] ξανατρέχει πάνω στη φρέσκια τιμή.
  /// Γι' αυτό πρέπει να είναι **καθαρή** — να αλλάζει μόνο ό,τι άγγιξε ο
  /// χρήστης και να μη στηρίζεται σε τίποτα που αλλάζει ανά κλήση.
  ///
  /// Επιστρέφει ό,τι όντως αποθηκεύτηκε: ο καλών ενημερώνει την οθόνη του με
  /// αυτό, όχι με τη δική του παλιά εικόνα.
  Future<String> updateSetting(
    ...
    final e = executor ?? db;
    late String next;
    for (var attempt = 0; attempt < _updateAttempts; attempt++) {
      final current = await getSetting(key, executor: e);
      next = change(current);
      if (await compareAndSetSetting(key, current, next, executor: e)) {
        return next;
      }
    }
    // Η αλλαγή γράφεται τότε χωρίς δέσμευση, αλλά πάνω στην τελευταία φρέσκια
    // εικόνα: ό,τι χειρότερο μπορεί να χαθεί είναι εγγραφή κλάσματος
    // δευτερολέπτου, ενώ το να μη γραφτεί καθόλου η επιλογή του χρήστη είναι
    // σίγουρη ζημιά.
    await saveSetting(key, next, executor: e);
    return next;
  }
}
