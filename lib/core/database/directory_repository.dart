...
class DirectoryRepository {
  ...
  /// Τηλέφωνα 1↔1 για τους χρήστες που πρόκειται να διαγραφούν (για διάλογο F2-02).
...
''');
  ...
  Future<int> updateEquipment(int id, Map<String, dynamic> values) async {
    ...
    await CallsRepository(db).rebuildSearchIndexForCallsByEquipmentId(db, id);
    return n;
  }
  ...
  /// Ενεργές (μη soft-deleted) εγγραφές καταλόγου — για διάλογο πριν την εισαγωγή Excel (F2-03).
  ...
  // --- Φύλλα κατόψης χάρτη κτιρίου (`building_map_floors`) ---
  ...
  Future<void> linkOrphanPhoneToUserForIntegrity({
    ...
    final existing = await _userPhoneNumbersOrdered(db, userId);
  ...
  Future<String> integrityUserLabel(DatabaseExecutor e, int? userId) async {
    if (userId == null) return '—';
    final row = await _userRowById(e, userId);
    if (row == null) return 'Χρήστης ID $userId [Ανύπαρκτος]';
    final name = _userDisplayNameFromRow(row);
