...
/// Το κείμενο της αντίστροφης μέτρησης όσο η εφαρμογή προσπαθεί να ανοίξει
/// τη βάση.
///
/// Ζει σε **ένα** σημείο γιατί το γράφουν τρεις: η αρχική δήλωση του βήματος,
/// ο χρονιστής που το ανανεώνει κάθε δευτερόλεπτο, και η οθόνη φόρτωσης που
/// το δείχνει μεγάλο στη μέση. Ως αντιγραμμένο κείμενο είχε ήδη αποκλίνει από
/// τα σωστά ελληνικά σε **όλα** τα αντίγραφα ταυτόχρονα — ακριβώς ό,τι κάνει
/// μια διόρθωση εύκολο να ξεχαστεί στο ένα από τα τρία.
String databaseOpenCountdownLabel(int secondsRemaining) {
  final unit = secondsRemaining == 1 ? 'δευτερόλεπτο' : 'δευτερόλεπτα';
  return 'Προσπάθεια ανοίγματος βάσης σε $secondsRemaining $unit';
}
