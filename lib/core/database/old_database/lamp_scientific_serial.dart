/// Η μορφή που παράγει **όντως** το υπολογιστικό φύλλο όταν καταπίνει έναν
/// μακρύ αριθμητικό σειριακό — και μόνο αυτή.
///
/// Τρεις όροι, ο καθένας αδιαπραγμάτευτος για μια αληθινή επιστημονική μορφή:
///
/// 1. **Ένα ψηφίο πριν την υποδιαστολή.** Το φύλλο κανονικοποιεί πάντα τη
///    μάντισσα: `3,10128E+11`, ποτέ `310128E+11`.
/// 2. **Πρόσημο στον εκθέτη.** Γράφεται πάντα `E+11`, ποτέ `E11`.
/// 3. **Εκθέτης χωρίς αρχικά μηδενικά**, ένα ως τρία ψηφία.
///
/// **Γιατί τόση αυστηρότητα (14/09/2026):** ο προηγούμενος κανόνας δεχόταν
/// μάντισσα οποιουδήποτε μήκους, εκθέτη χωρίς πρόσημο και αρχικά μηδενικά.
/// Έτσι σήμαινε ως «αλλοιωμένο» κάθε γνήσιο σειριακό που τυχαίνει να έχει
/// ένα `E` ανάμεσα σε ψηφία. Στη ζωντανή βάση των 3.155 σειριακών έβγαζε
/// **ένα** εύρημα, τον `310128E000079` ενός EDIMAX — και ήταν λάθος. Ο
/// εκθέτης `000079` θα σήμαινε αριθμό με ογδόντα ψηφία· κανένας σειριακός
/// δεν είναι τέτοιος, και καμία μηχανή δεν γράφει έτσι.
final RegExp _scientificSerialPattern = RegExp(
  r'^\s*[+-]?\d([.,]\d+)?[eE][+-][1-9]\d{0,2}\s*$',
);
