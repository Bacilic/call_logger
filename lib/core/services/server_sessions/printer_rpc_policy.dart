...
class PrinterRpcPolicyState {
  ...
  /// Τι λέει η **περιορισμένη προβολή εκτυπωτών** για συγκεκριμένο διακομιστή.
  ///
  /// Κάτω από ένα μήνυμα κρύβονταν δύο εντελώς διαφορετικές αιτίες:
  ///  * λείπει η ρύθμιση από **αυτόν** τον υπολογιστή — τότε περιορισμένοι
  ///    φαίνονται **όλοι** οι παλιοί διακομιστές·
  ///  * η ρύθμιση είναι περασμένη και δεν απαντά η ουρά **εκείνου** του
  ///    διακομιστή — τότε ο διπλανός διακομιστής δουλεύει κανονικά.
  ///
  /// Το παλιό κείμενο έστελνε πάντα στη ρύθμιση. Με τη ρύθμιση ήδη περασμένη
  /// η οδηγία ήταν αδιέξοδη: η κάρτα δίπλα έλεγε «πλήρης προβολή» ενώ ο
  /// διάλογος ζητούσε να ενεργοποιηθεί. Μετρημένο ζωντανά στους
  /// 192.168.13.82 (δουλεύει) και .83 (δεν απαντά η ουρά), από τον ίδιο
  /// υπολογιστή και την ίδια στιγμή.
  /// Το [suggestRestart] σβήνει την προτροπή για επανεκκίνηση της ουράς: μέσα
  /// στον ίδιο τον διάλογο της επανεκκίνησης ο χειριστής έχει ήδη το κουμπί
  /// μπροστά του, και μια οδηγία να πάει να το βρει στο μενού είναι γελοία.
  String limitedPrinterViewMessage({
    required String host,
    required bool showHint,
    int errorCode = 0,
    bool suggestRestart = true,
  }) {
    final code = errorCode == 0 ? '' : ' (κωδικός $errorCode)';
    final shared =
        'Περιορισμένη προβολή$code: η λίστα διαβάστηκε από τις ρυθμίσεις του '
        'διακομιστή. Βλέπεις ποιοι εκτυπωτές υπάρχουν, αλλά όχι τι περιμένει '
        'στις ουρές — και οι ενέργειες δεν είναι διαθέσιμες.';
    if (status == PrinterRpcPolicyStatus.enabled) {
      final onServer =
          '$shared Η ρύθμιση RPC αυτού του υπολογιστή είναι περασμένη, '
          'οπότε το πρόβλημα είναι στον ίδιο τον $host: η υπηρεσία ουράς '
          'εκτυπώσεων δεν απαντά — μπορεί να είναι σταματημένη.';
      if (!suggestRestart) return onServer;
      return '$onServer Δοκίμασε «Επανεκκίνηση ουράς εκτυπώσεων» από το μενού '
          'του διακομιστή.';
    }
    if (!showHint) return shared;
    return '$shared Η πλήρης προβολή ενεργοποιείται από τον Κατάλογο → '
        'Διάφορα → Διακομιστές, στην κάρτα «Κατάσταση αυτού του υπολογιστή».';
  }
